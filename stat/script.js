...
fetch("/stat/list.json")
  ...
  .then((json_res) => {
    ...
    for (let d = 0; d < json_res.domains.length; d++) {
      ...
      console.log(about)
      base_vmess = `{"add": "${subbd}.${urlObj.d}", "ps": "${subbd}.${urlObj.d}", "scy": "auto", "type": "", "sni": "", "path": "/ws/","port": 443, "v": 2, "host": "", "tls": "tls", "id": "${uuid}", "net": "ws"}`;

      li += `<li  class="list-group-item d-flex justify-content-between align-items-center">
      <p><small>${subbd}.${urlObj.d}</small> | ${added}</p>
